...
def translate_data(event=None):
    global sour_var,dect_var, tarns_fraem
    tarns_fraem = tk.Toplevel()
    sour_var = tk.StringVar()
    dect_var = tk.StringVar()
    tarns_fraem.geometry('450x250+500+200')
    tarns_fraem.title('Google Translation')
    tarns_fraem.resizable(0,0)
    
    trans_label = ttk.LabelFrame(tarns_fraem, text = 'Google  translator ')
    trans_label.pack()

    Laang = list(LANGUAGES.values())
    # label 
    ...
    #combobox 
    source_text = ttk.Combobox(trans_label, textvariable=sour_var, width = 20 )
    ...
    submit_btn = ttk.Button(tarns_fraem,text= 'Submit', command= translate_text)
...
transa.add_command(label= 'Translate English to hindi ', image = google_translation_icon , compound= tk.LEFT,  accelerator= 'Control - Enter ', command=translate_data)
...
count = 0 
for i in color_dict:
    color_theme.add_radiobutton(label=i,image= color_icons[count],variable= theme_choice,compound= tk.LEFT, command= change_theam  )
    count+=1
